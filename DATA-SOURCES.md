...
`effect-values.json`, `public/data/overrides.json` and `runes.json` are byte-identical.
...
## 53. Three questions after the merge (2026-08-14)

### 53.1 The name collision — swept before renaming

Two unrelated files were both called `overrides.json`: the merged override file in the protected
directory, and the ledger of champions whose wiki statistics disagreed with Data Dragon, which
`npm run fetch:data` regenerates.

**DEFINITION: occurrences of the literal string `overrides.json` in the repository, excluding
`node_modules/`, `.git/`, `dist/`, and the merged file's own contents.**

| | occurrences | files |
|---|---:|---:|
| the public ledger (`public/data/overrides.json`) | **17** | 5 |
| the merged override file (`curated/overrides.json`) | **0** | 0 |

The ledger also has a module and a test named after it (`scripts/fetch/overrides.ts`,
`overrides.test.ts`, 54 further mentions of the word), an entry in the generated
`public/data/manifest.json`, and a served URL constant.

**The merged file is named by nothing at all.** `loadCurated` scans the directory for any `.json`
and reads what it finds, so the name is not load-bearing anywhere. It is therefore the one to
rename, and the rename touches no code.

Renamed to **`curated/curated-data.json`**. The token `curated-data.json` appears nowhere else in
the repository, so it is greppable in isolation — which the old name was not, and that was the
real defect. Two files a human can tell apart but `grep` cannot is how the wrong one eventually
gets deleted.

One further ambiguity fixed while sweeping: DATA-SOURCES §43 said "`effect-values.json`,
`overrides.json` and `runes.json` are byte-identical" without qualifying which `overrides.json`.
It means the public one.

### 53.2 The build step the README describes and nothing performs — MEASURED, decision open

`curated/README.md` says the build "**copies** it into `public/data/curated/` to serve". Nothing
does. There is no reference to the protected directory in `vite.config.ts`, no copy of
`CURATED_DIR` anywhere, and `public/data/curated/` does not exist.

**What actually feeds the site today:** `public/data/abilities/`, **173 per-champion files
totalling 1.6 MB**, generated by `scripts/build-ability-files.ts` — which reads
`build/proposed-curated/abilities/batch-01.json`, the **harvester's draft**, not the protected
file. It is also not part of `npm run build`; it is run by hand and its output is committed.

**A whole-file copy would be the wrong shape**, which is why the instinct to "make the copy step
exist" needs one adjustment rather than a straight implementation:

- the protected file is **1,037,879 bytes in one piece**. Serving it means every visitor
  downloads all 173 champions to calculate one matchup;
- the per-champion split exists precisely so the interface fetches one champion at a time;
- so the honest form is **not a copy — it is re-pointing `build-ability-files.ts` at the
  protected file instead of the draft.** Same 173 outputs, same shape, sourced from the file
  that is guarded rather than the one that is not.

**THE ONE COST, MEASURED. DEFINITION: ability entries present in the harvester's draft batch and
absent from the merged override file, by key (champion + form / slot / ability name).**

**18 entries — and every one of them is already `incomplete`.** They are the 18 refused at merge:
16 for two damage rows sharing a component id, 2 for a `stacks` ratio naming no counter (Nasus Q,
Smolder P). Because `resolveDamage` gives an `incomplete` entry zero damage whatever components it
holds, **re-pointing changes no damage figure anywhere.**

What it changes is a sentence. Those 18 would go from "incomplete, and here is why" to "nothing
has been harvested for this champion's E slot" — which is **false**: something was harvested and
a gate refused it. So the re-point should carry the refusal list (`merge-refusals.json` already
holds exactly this, with each entry's real reason) rather than let the 18 fall off the edge.

Affected: Akali E, Anivia Q, Blitzcrank R, Evelynn Q, Graves Q, Kled Q, Malzahar R, Mel E, Nasus
Q, Sejuani W, Smolder P, Smolder Q, Sylas Q, Talon W, Twisted Fate W, Vel'Koz W, Vex R, Viktor E.

### 53.3 The rider question — both options sized, decision open

**21 of the 43 stored item effects reach their target on another instance** rather than standing
alone: 15 on-hit and 6 spellblade. 18 of the 21 are `derived` and ready; 3 are `incomplete`.
(§52.2's figure of 18 counted only the ready ones — same population, different filter.)

The question is whether a rider becomes its own row in the breakdown, or is folded into the
instance that carried it. It is visible to the reader either way, so it is a decision, not an
implementation detail. **Two measurements settle more of it than expected, and both point the
same way.**

**MEASUREMENT 1 — 10 of the 21 riders deal MAGIC damage**: Ardent Censer, Dusk and Dawn,
Guinsoo's Rageblade, Lich Bane, Nashor's Tooth, Rapid Firecannon, Statikk Shiv, Stormrazor,
Terminus, Wit's End. A basic attack deals physical. Folding a magic rider into it makes that
instance **mixed** — and `ENGINE_EXCLUSIONS` item A states that an instance dealing two types
gets **no `resistanceSteps` at all**, because the contract carries one four-step breakdown and a
mixed instance meets two. So folding would **remove the resistance working from the most common
instance in the game**, for 10 of the 21 riders.

**MEASUREMENT 2 — crit multiplies the WHOLE instance.** `combo.ts` applies the critical-strike
multiplier to every type's share of an instance, and `AbilityComponent` carries no per-component
crit eligibility. A basic attack can crit; an on-hit effect generally does not. Folding a rider
into a critting basic attack therefore **multiplies the rider too** — a silent overstatement of
exactly the kind this project exists to prevent, and one no test would catch because the
arithmetic is internally consistent.

| | **Own row** | **Folded into the carrier** |
|---|---|---|
| resistance working | kept for both | **lost on 10 of 21** |
| crit correctness | correct as-is | **wrong unless the contract gains per-component crit** |
| contract change | none | `AbilityComponent` needs a crit field; `InstanceResult` needs per-type resistance steps |
| breakdown rows | more rows — a basic attack with three on-hit items is 4 rows | fewer rows |
| burndown bars | one bar per rider | one bar per carrier |
| link format | no change — a rider is not a step the user places | no change |
| reader's mental model | "Nashor's Tooth did 15 magic" — matches the game's own combat text | "the attack did 82" — closer to what the health bar shows |

**Own row is the cheaper and the more correct option**, and folding is only viable behind two
contract changes that would each need their own justification. The genuine argument for folding
is the second-to-last row: a reader watching a health bar sees one drop, not four. That is a
presentation question the burndown could answer by grouping rows visually without the engine
